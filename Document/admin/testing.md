...
## 隔离主从 + PostgreSQL checkpoint E2E
...
脚本使用 `docker-compose.yml` 和 `docker-compose.admin-e2e.yml`，不会迁移或写入当前开发库。它覆盖空库升级、migration 往返、PostgreSQL checkpoint 读取/清理、受控用户/文件写入、逐目标审计、主从追平和普通用户回归。
