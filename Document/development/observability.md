...
当前仓库还没有共享 JSON 日志运行时。各入口分别调用 Python `logging.basicConfig()`，MCP server 还使用本地 `FileHandler`。本文件完成第 1.1 步的契约冻结和现状盘点；第 1.2 步才把旧 `LogRecord` 转换为 JSON、接入五类进程并改造 MCP stderr 传输，第 1.3 步才接入 Alloy/Loki/Grafana。

当前代码中的旧日志不因为本文创建而被伪造分类。第 1.2 步迁移旧记录时，无法可靠判断的 `event_code` 和 `category` 必须保持 `null`；只有新增的受管事件才能宣称满足完整 v1 契约。
...
| --- | --- | --- | --- |
| `web` | `CausalAgent.py`、`app/__init__.py`；根 logger 默认流式输出，数据库 readiness 失败还 `print` | Web 启动、数据库检查、聊天/文件/认证/管理员路由日志；部分消息内插用户名、会话/用户 ID、标题和异常原文 | 统一为 JSON stderr；ID 放字段，标题、异常和数据库错误只保留脱敏摘要 |
| `worker` | `app/agent/worker/bootstrap.py`、`runtime.py`、`execution.py`；入口调用 `basicConfig(force=True)` | slot、Job、worker、工具和执行异常；异常使用 `exc_info=True` 的路径较多 | 增加稳定启动事件；清理异常栈和 Job 上下文，第二阶段再做业务布点 |
| `monitor` | `Database/monitor_worker.py`、`Database/monitoring.py`、`app/db.py` | 快照更新、慢查询、从库回退、readiness 和采集异常 | 慢查询只保留摘要/耗时/原因码，不把 SQL 正文直接写入受管日志 |
| `maintenance` | `Database/bootstrap.py`、`Database/database_init.py`、`Database/checkpoint_setup.py`、`Database/checkpoint_cleanup_worker.py`、`Database/audit_before_db_upgrade.py` | bootstrap、checkpoint setup、cleanup Job ID/attempt 和数据库初始化结果；部分入口输出 stdout，`database_init.py` 还写 `database_init.log` | 统一维护进程 stderr；保留脱敏的状态、数量和原因码，不采集数据库容器自身日志 |
| `mcp` | `Agent/CausalAgentMCP/mcp_server.py` 作为 worker 内 stdio 子进程运行；当前使用 `mcp_server.log` | 工具读取文件的字节数、工具失败栈和启动/关闭消息 | 第 1.2 步删除本地 `FileHandler`，应用日志只进 stderr；stdout 只保留 MCP 协议 |

### 3.1 `print`、文件日志和非服务输出

- `app/__init__.py` 的 readiness 异常 `print` 可能把异常原文写到标准输出；应在第 1.2 步改为受管错误事件，不能暴露连接信息。
- `Database/database_init.py` 的 CLI 提示是人工命令行输出，同时配置了 stdout 和 `database_init.log`；它不是 JSON 日志，且文件日志应迁移到维护进程 stderr。
...
- `Agent/CausalAgentMCP/mcp_server.py` 的 `mcp_server.log` 是当前唯一明确的 MCP 应用文件日志；MySQL `/var/lib/mysql/mysql-slow.log` 属于数据库引擎日志，第一阶段明确不采集。

### 3.2 敏感内容、重复和异常栈风险

静态扫描确认以下位置需要在第二阶段逐项改造或加测试：
...
5. `CausalAgent.py`、worker、monitor、bootstrap、checkpoint cleanup 和数据库脚本分别配置根 logger；重复初始化可能产生重复 handler 或不同输出流，需由共享运行时幂等收敛。
6. `Database/database_init.py` 和 `app/db.py` 当前会记录 host、database 名称或连接异常摘要；这些连接元数据不能进入新的受管日志字段，失败只能保留稳定原因码。

## 4. 第二阶段日志布点清单

下表只定义候选事件和允许的最小字段，不在第 1.1 步改写业务日志。事件码落地时必须继续遵守本文件的格式、分类和禁止项。
...
- 第 1.2 步验收：UTC/Unicode、上下文隔离、线程与异步任务隔离、递归脱敏、各级截断、异常栈清理、重复初始化、序列化失败和 MCP stdout/stderr 必须有单元测试。
