...
2026.7.30
- 【内容新增】
  - DirectLiNGAM 已作为独立 MCP 工具 `causal_direct_lingam` 接入，用户明确点名 DirectLiNGAM 时 planner 会确定性选择该工具。
  - DirectLiNGAM runner 已复用 causal-learn 内置 `DirectLiNGAM(measure="pwling")`，并将非零带权系数写入 causal-learn `Dag` 后生成统一前端因果图。
  - 后处理已支持 `matrix_convention="target_to_source"`，环路检测和断边会按 DirectLiNGAM 的 `B[target, source]` 方向处理任意非零系数。
- 【验证】
  - 新增 DirectLiNGAM MCP wrapper smoke，锁定 Python 3.11 镜像中已跑通 `causal_direct_lingam -> runner -> DirectLiNGAM -> 带权图结果` 最小链路。
  - 相关 runner、MCP parser、planner、后处理和最终图选择测试已通过。
  - 补充 DirectLiNGAM 报告上下文和 MCP 阶段链路测试，覆盖显式工具选择、结果解析和进入后续 RAG/报告链的路由。
...
2026.7.31
- [内容新增]
  - 修复 DirectLiNGAM 最小链路：恢复 `run_direct_lingam_analysis` 的真实拟合路径，固定 `measure="pwling"`，并将 `B[target, source]` 的非零系数转换为统一 `Dag` / vis-network 图结果。
  - 将 `causallearn` 的 PC / Endpoint 相关导入改为延迟加载，避免未安装相关依赖时阻断 DirectLiNGAM runner 的导入。
- [验证]
  - `tests/test_direct_lingam_analysis.py`、`tests/test_postprocess_revision_flow.py`、`tests/test_agent_state_routing.py`、`tests/test_mcp_result_contract.py`、`tests/test_final_result_graph_selection.py` 通过。

---
2026.8.2
- 【BUG修复】
  - 修复 `fold` 将 LLM 返回的空白、`none`、`null` 字符串哨兵误判为真实文件名或列名的问题；未指定 target/treatment 的结构发现请求可继续进入预处理和 MCP。
  - 修复 LangGraph interrupt 只检查 `state.values` 导致暂停状态被包装成伪 `final_result` 的问题；现按 `tasks[*].interrupts` 判断恢复，并以流式 `__interrupt__` update 兜底。
  - 保证同一轮只产生 `interrupt` 或 `final_result` 一个终态事件，下一条用户消息可通过 `Command(resume=...)` 恢复 checkpoint。
- 【验证】
  - 定向回归 `67 passed, 1 warning`，覆盖空值边界、真实 fold 校验、InMemorySaver interrupt/resume、update 兜底和 worker 终态持久化。
  - 本机完整测试集 `108 passed, 1 warning`。
  - Python 3.11.15、LangGraph 1.2.1、langgraph-checkpoint 4.1.1 worker 镜像 smoke 通过。
  - Web/MySQL 全链路待环境恢复后补充；验证时主从 MySQL 已退出且 app 持续重启，未改动现有数据库拓扑。

---
2026.8.2
- 【文档新增】
  - 新增面向项目组分享的 DirectLiNGAM 独立 HTML 介绍页，覆盖简化原理、四步流程、典型应用、选型理由、假设自检和 CausalChat 实际接入链路。
  - README 文档索引增加 DirectLiNGAM 算法介绍入口。
- 【验证】
  - 完成 HTML 结构、页面内锚点、重复 ID 与 JavaScript 语法静态检查；当前无可用浏览器实例，桌面与移动端实际渲染核对待补充。
