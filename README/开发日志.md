...
2026.7.16
- 【内容新增】：
  - 完成多模态知识摄取规划 P08：新增默认关闭的视觉语义描述适配器、OpenAI-compatible Responses 生产 port、确定性 fake、内容哈希缓存、远程授权与图片调用上限。
  - 图表、流程图、普通照片和因果图统一转换为可追溯文字片段；未知方向连接产生阻塞性人工复核问题，视觉失败不阻断 OCR 或正文摄取。
  - 支持 PDF 页面或内嵌图片保留原 PDF 来源与 locator，并显式禁用远程响应存储。
- 【验证】：
