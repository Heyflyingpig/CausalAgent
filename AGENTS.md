...
- 每个 worker slot 会独占一组 MCP server process、一个 `ClientSession` 和一个编译好的 Agent graph；真实执行单元是 slot，不是 Flask 请求线程。graph 编译前会把该 slot 的 MCP `list_tools()` 动态包装成 LangChain tools。
