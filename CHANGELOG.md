...
- 【Runtime readiness】：新增不读取原始 PDF 的冻结来源身份校验；生产 Agent worker 仍校验 active pointer、manifest 哈希、embedding、版本、collection 和向量目录。
- 【发布门禁】：构建、evaluate、gate-check、publish 和 isolated staged gate 继续使用受控目录内的严格 PDF 哈希校验；运行期与构建期来源依赖分离。
