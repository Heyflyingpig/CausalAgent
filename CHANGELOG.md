...
---
2026.8.19
- 【日志管理第一阶段 1.3：修复 Alloy 配置语法】
  - 修复 `stage.json.expressions` 和 `stage.labels.values` 两个多行 map 缺少字段分隔逗号、导致 Alloy 报 `missing ',' in field list` 并退出的问题。
  - 新增多行 map 字段分隔符回归测试，并将固定版本镜像的 `alloy validate` 加入首次启动和配置变更后的强制验证步骤。
  - 当前环境 Docker 守护进程未运行，已完成静态与回归测试；固定镜像的真实 `alloy validate` 仍需在可用 Docker 环境执行。
