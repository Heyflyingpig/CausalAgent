...
def print_summary(entries: List[Dict[str, Any]]) -> None:
    ...
    write_cli_output("version | install | mode | core | wrappers | metrics(deprecated) | collections | current_internal")
    write_cli_output("-" * 100)
    for entry in entries:
        compact = summarize_probe(entry)
        if not compact["install_ok"]:
            write_cli_output(f"{entry['version']} | fail | - | - | - | - | - | -")
            continue
        for mode in ["plain", "with_vertexai_shim"]:
            values = compact.get(mode, {})
            write_cli_output(" | ".join(
                    [
                        entry["version"],
                        "ok",
                        mode,
...
def main() -> int:
    ...
    write_cli_output("\nFull JSON:")
    write_cli_output(json.dumps(output, ensure_ascii=False, indent=2))
    if args.json_output:
