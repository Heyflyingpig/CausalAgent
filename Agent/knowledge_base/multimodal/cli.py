...
def main() -> int:
    ...
    elif args.command == "ingest": result = service.ingest(args.source or [str(path) for path in production_source_paths()], allow_remote_data=args.allow_remote_data, max_images=args.max_images, retry_failed=args.retry_failed, retry_generation=args.retry_generation, retry_from_index_version=args.retry_from_index_version, reuse_local_from_index_version=args.reuse_local_checkpoints_from)
    elif args.command == "run": result = service.run(args.source or [str(path) for path in production_source_paths()], allow_remote_data=args.allow_remote_data, max_images=args.max_images, retry_failed=args.retry_failed, retry_generation=args.retry_generation, retry_from_index_version=args.retry_from_index_version, reuse_local_from_index_version=args.reuse_local_checkpoints_from, timeout_seconds=args.timeout_seconds, cancel_check=(lambda: Path(args.cancel_file).exists()) if args.cancel_file else None, publish_on_pass=args.publish)
