...
- **隔离评测报告编辑与清理**：报告入口统一命名为“报告编辑”，可在同一页面切换流程、检索和 Ragas Markdown；删除已结束评测后，后端会移除对应 `tmp/rag_eval/runs/<run_id>/` 目录，历史与对比接口不再返回该运行。对超过无事件活动窗口的失活评测，用户确认后可强制删除；仍有活动迹象的运行禁止删除，摄取运行和 staged index 保留。
...
- **隔离评测运行清理**：工作台的“本次工作索引”下拉框只展示已完成的 staged 摄取运行；`DELETE /api/rag_eval/isolated/ingestion-runs/<run_id>` 会先检查候选题、RAG 试跑、评测、治理运行、正式 Gold 和 production active pointer 的引用，存在引用时拒绝删除。候选题、调参集治理和 Gold 治理运行也支持删除终态目录；运行中任务必须先取消并等待终态。
...
- **隔离评测生产接口与队列事实（2026-08-21）**：正式通用 run lifecycle 是 `GET /api/rag_eval/isolated/runs/<run_id>`、`/result`、`/artifacts/<artifact_name>`、`/stream` 与 `POST /cancel`；各具体 run 类型的状态/结果/产物/SSE/取消路径仍保留，但会返回 `Deprecation: true` 和指向通用路径的 successor `Link`。当前完整路由、请求字段和迁移说明见 `Document/api/rag-eval.md`；历史清单不再作为独立现行文档维护，删除具体接口前仍必须结合访问日志、版本公告和弃用窗口，且鉴权不在本轮范围。
...
- **隔离评测验收分层**：`config/rag_eval_production_acceptance_matrix.json` 与安全 runner 将检查分为 contract、integration、production 三层：contract 仅非破坏性白名单检查，integration 使用临时 fixture，production 仅允许显式确认后的只读 readiness；它们不等同于已执行真实生产摄取、评测或发布。
...
- **自动评测集生成与可选审核闭环**：`/rag_eval` 的“自动生成评测集”页面调用 `POST /api/rag_eval/isolated/candidate-runs`，默认使用 Ragas 0.4.3 `generate_with_chunks()`；生成进度、取消、逐题编辑和三态审核通过 SSE/隔离产物保存。生成的 `generated_candidate` 可在审核前直接评测，逐题复核只补充质量标记。页面显示用户定义的知识源名称，索引号只显示短别名但保留完整值用于复制和审计；审核编辑只写新的 reviewed candidate revision，原候选文件拒绝覆盖。`POST /api/rag_eval/gold-v2/freeze` 仍仅在 Pearl 24 题、候选 48 题和完整 approved 审核清单齐备时冻结内部 Gold v2。
...
摄取状态持久化在 `tmp/rag_eval/runs/<ingestion_run_id>/run.json`，staged index、图片、Chroma 和 manifest 保存在同一运行目录；页级 checkpoint 使用该 index 下的 `checkpoints.sqlite3`，便于失败后按页复用。GET /api/rag_eval/isolated/ingestion-runs 会枚举这些状态，页面在 localStorage 丢失、首次请求失败或点击顶部刷新后，都会尝试恢复最近的运行中或 staged 状态。隔离评测长任务的队列状态在 MySQL `rag_eval_jobs`，具体产物仍在各自的 `tmp/rag_eval/runs/<run_id>/`；来源位于 `tmp/rag_eval/sources/`，制品位于 `tmp/rag_eval/artifacts/`，运行目录和上传来源目录已加入 Git 忽略，不应提交到仓库。worker 重启后继续领取尚未开始的 queued 任务；已经失联的 running 任务按 heartbeat 超时失败收敛，不自动重放可能产生外部调用的工作。
